...
class tpl
{
    ...
    private static function get($tn)
    {
        ...
        {
                return msg($Lang['_error'], sprintf($Lang['_failed_to_get_content'], $tn), 'error', true);
        }
    }
    public static function parse($t, $a=null)
    {
        global $Lang;
        //return preg_replace_callback('#\{([a-z0-9\-_]*?)\}#Ssi', function($m) use ($a) {return isset($a[$m[1]])? $a[$m[1]]:$m[1];}, tpl::get($t));
        return preg_replace_callback('#\{([a-z0-9\-_]*?)\}#Ssi', function($m) use ($a,$Lang) {return isset($a[$m[1]])? $a[$m[1]]:((isset($Lang[$m[1]]))?$Lang[$m[1]]:$m[1]);}, tpl::get($t));
    }
}