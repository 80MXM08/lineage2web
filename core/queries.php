...
$q = [
    'core' => [
        ...
        'UPDATE_CONFIG' => "UPDATE `config` SET `value`=':val' WHERE `name`=:name AND `type`=:type LIMIT 1;",
        ...
        //Messages
        'NEW_MESSAGES' => "SELECT * FROM messages WHERE receiver=:acc AND unread='yes';",
        'SENT_MESSAGES' => "SELECT * FROM `messages` WHERE `sender`=:sender;",
        'RECEIVED_MESSAGES' => "SELECT * FROM `messages` WHERE `receiver`=:receiver;",
        //WebShop
        ...
        'INSERT_LOG'=>'INSERT INTO `log` (`account`, `type`, `subType`, `comment`) VALUES (:acc, :type, :sub, :comment);',
       ],
    'ls' => [
        'CHECK_LOGIN' => "SELECT * FROM `accounts` WHERE `login`=:name AND `password`=:pass;",
        'CHECK_EXISTS' => "SELECT * FROM `accounts` WHERE `login`=?;",
        ...
        'GET_ACC_COUNT' => 'SELECT count(*) as count FROM `accounts`;',
        'GET_ACC_DATA_VARS' => "SELECT `var`, `value` FROM `account_data` WHERE account_name=:account;",
        'CHECK_ACC_DATA_VAR' => "SELECT * FROM `account_data` WHERE `account_name`=:account AND `var`=:var;",
        'UPDATE_ACC_DATA_VAR' => "UPDATE account_data SET `value` = :val WHERE `account_name` = :account AND `var`=:var;",
        ...
        'UPDATE_ACC_VAR' => "UPDATE accounts SET `:var`=:val WHERE login=:acc;",
       ],
    'cs' => [
    ],
    'gs' => [
